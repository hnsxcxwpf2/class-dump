...
void print_usage(void)
{
    fprintf(stderr,
            ...
            "\n"
            "  where options are:\n"
            "        -a             show instance variable offsets 显示实例变量偏移地址\n"
            "        -A             show implementation addresses 显示方法实现地址\n"
            "        --arch <arch>  choose a specific architecture from a universal binary (ppc, ppc64, i386, x86_64, armv6, armv7, armv7s, arm64)从通用二进制文件中选择特定的体系结构 \n"
            "        -C <regex>     only display classes matching regular expression 仅显示与正则表达式匹配的类 \n"
            "        -f <str>       find string in method name 在方法名中查找字符串\n"
            "        -H             generate header files in current directory, or directory specified with -o  在当前目录或用-o指定的目录中生成头文件 \n"
            "        -I             sort classes, categories, and protocols by inheritance (overrides -s) 按继承对类、类别和协议排序（覆盖-s） \n"
            "        -o <dir>       output directory used for -H\n"
            "        -r             recursively expand frameworks and fixed VM shared libraries 递归地扩展框架和固定的vm共享库 \n"
            "        -s             sort classes and categories by name\n"
            "        -S             sort methods by name\n"
            "        -t             suppress header in output, for testing 不生成头文件,用于测试\n"
